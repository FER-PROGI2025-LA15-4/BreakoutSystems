...
@app.route('/api/start-room-payment', methods=['POST'])
@login_required
def start_room_payment():
    ...
    # Stripe iznos u centima (npr. 60.00 EUR -> 6000)
...
@app.route('/api/confirm-room-payment', methods=['POST'])
@login_required
def confirm_room_payment():
    ...
    try:
        ...
        if session.payment_status == 'paid' and session.metadata.get('tip_placanja') == 'rezervacija_sobe':
...
def test_stripe_payment():
    ...
    try:
        checkout_session = stripe.checkout.Session.create(
            payment_method_types=['card'],
            client_reference_id=current_user.username, # DODAJ OVO da confirm_payment zna tko je korisnik
            metadata={
                'tip_pretplate': tip_pretplate # OVO JE KLJUČNO
            },
            line_items=[{
                'price_data': {
                    'currency': 'eur',
